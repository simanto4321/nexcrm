...
export default function DashboardPage() {
  ...
  return <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {stats.map(s => <ZohoCard key={s.label} className="p-5">
      ...
      <ZohoCard className="p-5 mb-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {Object.entries(data.deals_by_stage).map(([stage, count]) => <div key={stage} className={`border-l-4 ${stageColors[stage] || 'border-l-gray-400'} bg-[#fafbfc] rounded-r-lg p-4`}>
      ...
      <ZohoCard className="p-5">
        ...
        {!data.recent_activity?.length ? <p className="text-sm text-[#616e88]">No recent activity yet.</p> : <ul className="space-y-3">
            {data.recent_activity.map((a, i) => <li key={`${a.kind}-${a.entity_id}-${i}`} className="flex items-start gap-3 border-b border-black/5 pb-3 last:border-0">
