...
export default function ContactsPage() {
  ...
  const filtered = contacts.filter(c => {
    ...
    return c.name.toLowerCase().includes(q) || (c.email || '').toLowerCase().includes(q) || (c.phone || '').toLowerCase().includes(q) || (c.status || '').toLowerCase().includes(q) || (c.notes || '').toLowerCase().includes(q);
  });
  function openCreate() {
    setEditing(null);
    setForm(emptyForm);
  ...
  return <div>
      <PageTitle title="Contacts" subtitle={`${filtered.length} contact${filtered.length === 1 ? '' : 's'}${query ? ` matching "${query}"` : ''}`} action={<button type="button" onClick={() => showForm ? (setShowForm(false), setEditing(null)) : openCreate()} className="btn-zoho">
      ...
      {showForm && <ZohoCard className="p-5 mb-6">
          ...
          <form onSubmit={onSubmit} className="grid sm:grid-cols-2 gap-4">
            <ZohoInput label="Name" required value={form.name} onChange={e => setForm({
          ...form,
          name: e.target.value
        })} />
            <ZohoInput label="Email" type="email" value={form.email} onChange={e => setForm({
          ...form,
          email: e.target.value
        })} />
            <ZohoInput label="Phone" value={form.phone} onChange={e => setForm({
          ...form,
          phone: e.target.value
        })} />
            <ZohoSelect label="Status" value={form.status} onChange={e => setForm({
          ...form,
          status: e.target.value
            ...
            <label className="sm:col-span-2 block">
              ...
              <textarea className="zoho-input min-h-[80px]" value={form.notes} onChange={e => setForm({
            ...form,
      ...
      <ZohoCard className="overflow-hidden">
        {loading ? <LoadingBlock /> : filtered.length === 0 ? <p className="p-8 text-center text-[#616e88]">No contacts found.</p> : <table className="w-full zoho-table">
            <thead>
              <tr>
                <th>Contact Name</th>
                <th>Email</th>
