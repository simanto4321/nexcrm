...
import { api, DEAL_STAGES } from '../api/client';
...
export default function PipelinePage() {
  ...
  return <div>
      <PageTitle title="Deals" subtitle="Drag deals between pipeline stages." action={<button type="button" onClick={() => showForm ? setShowForm(false) : openCreate()} className="btn-zoho">
            {showForm ? 'Cancel' : '+ New Deal'}
          </button>} />
      {msg && <p className="text-sm text-green-700 mb-4">{msg}</p>}

      {showForm && <ZohoCard className="p-5 mb-6">
          ...
          <form onSubmit={onSubmit} className="grid sm:grid-cols-3 gap-4">
            <ZohoSelect label="Contact" value={form.contact_id} onChange={e => setForm({
          ...form,
          contact_id: e.target.value
            ...
            <ZohoInput label="Value ($)" type="number" min="0" required value={form.value} onChange={e => setForm({
          ...form,
          value: e.target.value
        })} />
            <ZohoSelect label="Stage" value={form.stage} onChange={e => setForm({
          ...form,
      ...
      <DndContext sensors={sensors} onDragStart={e => setActiveId(Number(e.active.id))} onDragEnd={onDragEnd}>
        <div className="flex gap-3 overflow-x-auto pb-4">
          {DEAL_STAGES.map(stage => <StageColumn key={stage} stage={stage} deals={filtered.filter(d => d.stage === stage)} contactMap={contactMap} onEdit={openEdit} onDelete={onDelete} />)}
        </div>
        <DragOverlay>
          {activeId ? <DealCard deal={filtered.find(d => d.id === activeId)} contactMap={contactMap} dragging /> : null}
        </DragOverlay>
      </DndContext>
    </div>;
}
function StageColumn({
  ...
  return <div ref={setNodeRef} className={`flex-shrink-0 w-60 rounded-lg border bg-white ${isOver ? 'ring-2 ring-[#338cf0]' : 'border-[#e4e7ec]'}`}>
      ...
      <div className="p-2 space-y-2 min-h-[300px] bg-[#fafbfc]">
        {deals.map(d => <DraggableDeal key={d.id} deal={d} contactMap={contactMap} onEdit={onEdit} onDelete={onDelete} />)}
      </div>
    </div>;
}
...
function DealCard({
  ...
  return <ZohoCard className={`p-3 cursor-grab ${dragging ? 'shadow-lg ring-2 ring-[#e42527]/30' : ''}`}>
      ...
      {onEdit && onDelete && !dragging && <div className="flex gap-1 mt-2" onPointerDown={e => e.stopPropagation()}>
