...
export default function SettingsPage() {
  ...
  function load() {
    setLoading(true);
    Promise.all([api.get('/email/config'), api.get('/telegram/status')]).then(([emailRes, tgRes]) => {
  ...
  return <div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <ZohoCard className="p-5">
          ...
          {telegram?.bot_configured && telegram.bot_username && <p className="text-xs text-green-700 mb-3">Bot active: @{telegram.bot_username}</p>}
          {!telegram?.bot_configured && <p className="text-xs text-amber-700 mb-3">Telegram bot is not ready yet. Contact your platform administrator.</p>}
          <p className="text-sm text-[#616e88] mb-4">
