...
import { api, type TeamInvite, type TeamMember } from '../api/client'
...
export default function TeamPage() {
  ...
  const [mode, setMode] = useState<'member' | 'invite'>('member')
  ...
  function load() {
    ...
    const jobs: Promise<unknown>[] = [
      api.get<TeamMember[]>('/team/members').then((r) => setMembers(r.data)),
    ]
    if (isAdmin) {
      jobs.push(api.get<TeamInvite[]>('/team/invites').then((r) => setInvites(r.data)).catch(() => setInvites([])))
    }
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      if (mode === 'member') {
        ...
        await api.post('/team/invites', { email: form.email, role: form.role })
  ...
  return (
    <div>
      <PageTitle
        title="Team"
        subtitle="Manage workspace members and pending invites."
        action={
          isAdmin ? (
            <button
              type="button"
              className="btn-zoho"
              onClick={() => { setShowForm(!showForm); setMode('member') }}
            >
      ...
      {showForm && isAdmin && (
        <ZohoCard className="p-5 mb-6">
          ...
          <form onSubmit={onSubmit} className="grid sm:grid-cols-2 gap-4">
            ...
            <ZohoInput label="Email" type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
            {mode === 'member' && (
              <ZohoInput label="Temp password" type="password" required minLength={8} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
            )}
            <ZohoSelect label="Role" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>
      ...
      {isAdmin && (
        <ZohoCard className="overflow-hidden">
          ...
          {invites.length === 0 ? (
            <p className="p-6 text-sm text-[#616e88]">No invites yet.</p>
          ) : (
            <table className="w-full zoho-table">
              <thead>
                <tr>
              ...
              <tbody>
                {invites.map((i) => (
                  <tr key={i.id}>
                    ...
                    <td>
                      {i.status === 'pending' && (
                        <button type="button" className="text-xs text-red-600" onClick={() => cancelInvite(i.id)}>Cancel</button>
                      )}
                    </td>
                  </tr>
                ))}
