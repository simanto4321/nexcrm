...
export default function PlatformAdminPage() {
  ...
  const cards = [
    { label: '🏢 Workspaces', value: String(overview?.tenant_count ?? tenants.length) },
    { label: '✅ Active', value: String(overview?.active_tenants ?? 0) },
    { label: '👥 Users', value: String(overview?.total_users ?? 0) },
    { label: '📇 Contacts', value: String(overview?.total_contacts ?? 0) },
    { label: '💼 Deals', value: String(overview?.total_deals ?? 0) },
    { label: '📈 Pipeline', value: money(overview?.total_pipeline ?? 0) },
  ]
    ...
    <div className="min-h-screen deep-bg text-white">
      ...
      <div className="max-w-6xl mx-auto p-6 page-fade">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
          {cards.map((c) => (
            <ZohoCard key={c.label} className="p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {([
            ...
            <button
              ...
              className={`btn-zoho-secondary text-xs py-1.5 px-3 ${filter === key ? '!bg-[#e42527] !text-white !border-[#e42527]' : ''}`}
            >
              {label}
            </button>
          ))}
