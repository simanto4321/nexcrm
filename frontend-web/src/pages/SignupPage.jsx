...
export default function SignupPage() {
  ...
  return <AuthLayout title="🚀 Get started free" subtitle="Create your company workspace in minutes." footer={<><span className="text-[#616e88]">Have an account? </span><AuthLink to="/login">🔑 Sign in</AuthLink></>}>
      <form onSubmit={onSubmit} className="space-y-4">
        {error && <Alert message={error} tone="error" />}
        {fields.map(f => <ZohoInput key={f.key} label={f.label} type={'type' in f ? f.type : 'text'} required value={form[f.key]} onChange={e => setField(f.key, e.target.value)} />)}
