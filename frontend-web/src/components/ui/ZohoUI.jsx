...
export function Badge({
  ...
  return <span className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold ${tones[tone]}`}>{children}</span>;
}
export function LoadingBlock({
  label = 'Loading...'
}) {
  return <div className="flex items-center justify-center py-16 text-[#616e88] text-sm gap-3">
...
export function Alert({
  ...
  return <div className={`rounded-md px-4 py-3 text-sm mb-4 border ${tone === 'success' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
      {message}
    </div>;
}