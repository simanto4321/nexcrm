export function GlassCard({
  ...
  return <div className={`glass rounded-2xl ${hover ? 'transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]' : ''} ${className}`}>
      {children}
    </div>;
}
...
export function Badge({
  ...
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${tones[tone]}`}>
      {children}
    </span>;
}
export function LoadingScreen({
  ...
  return <div className="flex flex-col items-center justify-center py-24 gap-4">
...
export function AlertBanner({
  ...
  return <div className={`rounded-xl px-4 py-3 text-sm border ${tone === 'success' ? 'bg-emerald-500/10 border-emerald-500/25 text-emerald-200' : 'bg-rose-500/10 border-rose-500/25 text-rose-200'}`}>
      {message}
    </div>;
}