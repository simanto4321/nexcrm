...
const navItems = [
  ...
  { to: '/settings', label: 'Setup', icon: '⚙️', adminOnly: true },
]
  ...
  useEffect(() => {
    ...
    function poll() {
      api.get<{ unread: number }>('/notifications/unread-count')
        .then((r) => { if (alive) setUnread(r.data.unread) })
        .catch(() => {})
    }
    poll()
    ...
    <div className="app-shell flex min-h-screen deep-bg-app">
          ...
          {navItems.filter((i) => !i.adminOnly || isAdmin).map((item) => (
            <NavLink key={item.to} to={item.to} className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}>
              ...
              {item.to === '/notifications' && unread > 0 && (
                <span className="ml-auto text-[10px] font-bold bg-[#e42527] text-white rounded-full px-1.5 min-w-[1.25rem] text-center">
                  {unread > 9 ? '9+' : unread}
                </span>
              )}
            </NavLink>
      ...
      <div className="flex-1 lg:ml-56 flex flex-col min-h-screen">
        <header className="h-14 topbar-dark border-b flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30 gap-3">
          ...
          <NavLink
            to="/notifications"
            className="relative text-slate-300 hover:text-white text-sm px-2 py-1"
            title="Notifications"
          >
            🔔
            {unread > 0 && (
              <span className="absolute -top-1 -right-1 text-[9px] font-bold bg-[#e42527] text-white rounded-full w-4 h-4 flex items-center justify-center">
                {unread > 9 ? '9+' : unread}
              </span>
            )}
          </NavLink>
