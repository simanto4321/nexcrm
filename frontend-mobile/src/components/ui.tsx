...
export function PrimaryButton({
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.btn,
        (disabled || pressed) && { opacity: disabled ? 0.5 : 0.88, transform: [{ scale: pressed ? 0.98 : 1 }] },
      ]}
    >
...
export function GhostButton({ label, onPress, emoji }: { label: string; onPress: () => void; emoji?: string }) {
  return (
  ...
  return (
    <BrandBackground>
      <View style={styles.loading}>
        ...
        <Text style={[styles.subtitle, { marginTop: 12 }]}>Loading NexCRM…</Text>
      </View>
    </BrandBackground>
  )
}
...
export function Badge({
  ...
  text: string
  tone?: 'brand' | 'accent' | 'success' | 'muted' | 'warning'
}) {
  const map: Record<string, { bg: string; fg: string }> = {
    brand: { bg: 'rgba(228,37,39,0.22)', fg: colors.brandSoft },
    accent: { bg: 'rgba(59,158,255,0.22)', fg: colors.accent },
    success: { bg: 'rgba(34,197,94,0.22)', fg: colors.success },
    warning: { bg: 'rgba(245,158,11,0.22)', fg: colors.warning },
    muted: { bg: 'rgba(148,163,184,0.2)', fg: colors.muted },
  }
  const t = map[tone]
  return (
