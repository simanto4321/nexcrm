...
export default function DealsScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await api.get<Deal[]>('/deals')
      setItems(r.data)
    } finally {
      setLoading(false)
    }
  ...
  const filtered = useMemo(
    () => (stage === 'all' ? items : items.filter((d) => d.stage === stage)),
    [items, stage],
  )

  async function move(deal: Deal, next: string) {
    try {
      await api.put(`/deals/${deal.id}`, { stage: next })
      await load()
    } catch {
  ...
  return (
    <Screen>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12, maxHeight: 44 }}>
        ...
        {STAGES.map((s) => (
          <Pressable key={s} style={[styles.chip, stage === s && styles.chipOn]} onPress={() => setStage(s)}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Empty text="No deals in this stage" emoji={featureEmoji.deals} />}
        renderItem={({ item }) => (
          <Card>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 12 }}>
              {STAGES.filter((s) => s !== item.stage).map((s) => (
                <Pressable key={s} style={styles.move} onPress={() => move(item, s)}>
...
const styles = StyleSheet.create({
  ...
  chipText: { color: colors.muted, fontWeight: '600', fontSize: 12, textTransform: 'capitalize' },
  chipTextOn: { color: colors.white },
  row: { flexDirection: 'row', alignItems: 'center' },
  title: { color: colors.white, fontWeight: '700', fontSize: 16 },
  value: { color: colors.accent, fontWeight: '800', fontSize: 20, marginTop: 4 },
  move: {
  ...
  moveText: { color: colors.muted, fontSize: 12, fontWeight: '600' },
})
