...
export default function SettingsScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const [emailRes, tgRes] = await Promise.all([api.get('/email/config'), api.get('/telegram/status')]);
  ...
  async function saveTelegram() {
    try {
      await api.post('/telegram/register', {
        chat_id: chatId,
  ...
  return <Screen>
      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.brand} />} contentContainerStyle={{
      paddingBottom: 100
    }}>
        ...
        {!isAdmin ? <Card>
          ...
          borderColor: 'rgba(59,158,255,0.35)'
            ...
            <Card>
              ...
              {telegram?.bot_configured && telegram.bot_username ? <Text style={styles.ok}>Bot active · @{telegram.bot_username}</Text> : <Text style={styles.hint}>Telegram bot is not ready yet. Contact your platform administrator.</Text>}
        ...
        <Card>
          ...
          {[`${featureEmoji.contacts} Contacts & notes`, `${featureEmoji.deals} Deal pipeline`, `${featureEmoji.tasks} Tasks & follow-ups`, `${featureEmoji.alerts} Smart alerts`, `${featureEmoji.ai} AI assistant`, `${featureEmoji.telegram} Telegram notifications`, `${featureEmoji.email} Email notifications`].map(line => <Text key={line} style={styles.featureLine}>
              {line}
            </Text>)}
        </Card>
      </ScrollView>
