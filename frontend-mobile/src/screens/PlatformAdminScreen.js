...
export default function PlatformAdminScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const {
        data
      } = await api.get('/platform/tenants');
      setTenants(data);
    } catch {
      setTenants([]);
    } finally {
  ...
  if (token && isPlatform) {
    ...
    return <Screen>
        ...
        <FlatList data={tenants} keyExtractor={t => String(t.id)} ListEmptyComponent={<Empty text="No workspaces yet" emoji="🏢" />} contentContainerStyle={{
        paddingBottom: 40
      }} renderItem={({
        item
      }) => <Card>
            ...
            flex: 1
              ...
              <PrimaryButton label={item.status === 'active' ? 'Suspend workspace' : 'Activate workspace'} onPress={() => toggleStatus(item)} emoji={item.status === 'active' ? '⏸️' : '▶️'} />
            </Card>} />
      </Screen>;
  }
      ...
      flex: 1
        ...
        <ScrollView contentContainerStyle={styles.wrap} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.card}>
            ...
            <PrimaryButton label={busy ? 'Signing in…' : 'Sign In'} onPress={onLogin} disabled={busy} emoji={featureEmoji.admin} />
            <GhostButton label="Back to team sign in" emoji={featureEmoji.login} onPress={() => navigation.navigate('Login')} />
          </View>
        </ScrollView>
