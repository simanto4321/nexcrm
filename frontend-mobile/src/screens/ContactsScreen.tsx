...
export default function ContactsScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await api.get<Contact[]>('/contacts')
      setItems(r.data)
    } finally {
      setLoading(false)
    }
  ...
  async function create() {
    ...
    try {
      ...
      setOpen(false)
      setForm({ name: '', email: '', phone: '', status: 'lead', notes: '' })
      await load()
    } catch {
  ...
  return (
    <Screen>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Empty text="No contacts yet" emoji={featureEmoji.contacts} />}
        renderItem={({ item }) => (
          <Card>
...
const styles = StyleSheet.create({
  ...
  notes: { color: colors.muted, fontSize: 12, marginTop: 4 },
