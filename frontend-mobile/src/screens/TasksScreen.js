...
export default function TasksScreen() {
  ...
  return <Screen>
      ...
      <FlatList data={items} keyExtractor={t => String(t.id)} ListEmptyComponent={<Empty text="No tasks yet" emoji={featureEmoji.tasks} />} renderItem={({
      item
    }) => <Pressable onPress={() => toggle(item)}>
            <Card>
              <View style={styles.row}>
                <View style={[styles.check, item.status === 'done' && styles.checkOn]}>
                  {item.status === 'done' ? <Text style={{
              color: colors.white
            }}>✓</Text> : null}
                </View>
                <View style={{
            flex: 1
                  ...
                  {item.due_date ? <Text style={styles.due}>Due {new Date(item.due_date).toLocaleDateString()}</Text> : null}
                </View>
