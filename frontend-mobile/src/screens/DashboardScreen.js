...
export default function DashboardScreen() {
  ...
  const openDeals = Object.entries(data.deals_by_stage).filter(([s]) => s !== 'won' && s !== 'lost').reduce((a, [, n]) => a + n, 0);
  const stats = [{
    label: `${featureEmoji.contacts} Contacts`,
    value: String(data.total_contacts)
  }, {
    label: `${featureEmoji.deals} Open deals`,
    value: String(openDeals)
  }, {
    label: `${featureEmoji.tasks} Tasks`,
    value: String(data.pending_tasks)
  }, {
    label: `${featureEmoji.team} Team`,
    value: String(data.team_count ?? 0)
  }];
  return <Screen>
      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.brand} />}>
        ...
        <View style={styles.grid}>
          {stats.map(s => <Card key={s.label} style={styles.statCard}>
        ...
        <View style={styles.row}>
          <Card style={styles.half}>
            ...
            color: colors.accent
          }]}>{money(data.pipeline_value ?? 0)}</Text>
          </Card>
          <Card style={styles.half}>
            <Text style={styles.statLabel}>{featureEmoji.won} Won</Text>
            <Text style={[styles.statValue, {
            color: colors.success
          }]}>{money(data.won_value ?? 0)}</Text>
          </Card>
        </View>

        <Card>
          <Text style={styles.section}>{featureEmoji.deals} Deals by stage</Text>
          {Object.entries(data.deals_by_stage).map(([stage, count]) => <View key={stage} style={styles.stageRow}>
        ...
        <Card>
          ...
          {!data.recent_activity?.length ? <Empty text="No recent activity yet" emoji="📭" /> : data.recent_activity.slice(0, 8).map((a, i) => <View key={`${a.kind}-${i}`} style={styles.activity}>
