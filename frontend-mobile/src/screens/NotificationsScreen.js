...
export default function NotificationsScreen() {
  ...
  return <Screen>
      ...
      <FlatList data={items} keyExtractor={n => String(n.id)} ListEmptyComponent={<Empty text="No notifications yet" emoji={featureEmoji.alerts} />} renderItem={({
      item
    }) => <Card style={!item.is_read ? {
      borderColor: 'rgba(59,158,255,0.35)'
          ...
          flex: 1
            ...
            {!item.is_read ? <Pressable onPress={() => markRead(item.id)} style={{
        marginTop: 10
      }}>
